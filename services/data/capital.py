...
class CapitalClient:
    ...
    async def place_limit_order(self, epic: str, direction: str, size: float,
                                level: float, stop_loss: float,
                                take_profit: float) -> dict:
        """
        POST /workingorders/otc — creates a LIMIT working order at `level`.
        The order sits on Capital.com until price reaches `level` or we cancel it.
        Returns {status, deal_reference, response}.
        """
        try:
            payload = {
                "epic":           epic,
    ...
    async def get_working_orders(self) -> list:
        """GET /workingorders — returns all active working orders."""
        try:
            async with aiohttp.ClientSession() as s:
                r = await s.get(
                ...
                d = await r.json()
                orders = []
                for wo in d.get("workingOrders", []):
                    wod = wo.get("workingOrderData", {})
                    mkt = wo.get("marketData", {})
                    orders.append({
                        "deal_id":     wod.get("dealId"),
                        "epic":        mkt.get("epic"),
                        "direction":   wod.get("direction"),
                        "size":        wod.get("orderSize"),
                        "level":       wod.get("orderLevel"),
                        "stop_loss":   wod.get("stopLevel"),
                        "take_profit": wod.get("profitLevel"),
                    })
                return orders
        except Exception as e:
            print(f"Working orders fetch error: {e}")
            return []

    async def cancel_working_order(self, deal_id: str) -> dict:
        """DELETE /workingorders/otc/{deal_id} — cancel a working order."""
        try:
            async with aiohttp.ClientSession() as s:
                r = await s.delete(
