...
async def format_btc_signal(sig: dict) -> str:
    ...
    return (
        ...
        f"Setup:    {sig.get('setup_type','n/a').upper().replace('_',' ')}\n"
        f"Regime:   {sig.get('market_regime','n/a').upper().replace('_',' ')}\n"
        f"Confidence: {sig['confidence']}/100\n"
        f"Session: {sig.get('session_context','n/a')} ({sig.get('session_priority','')})\n\n"
        f"MARKET\n"
        f"Price:  {_p(sig['price'])} ({sig['change_pct']:+.3f}%)\n"
        f"RSI(1m): {sig['rsi']} — {sig['rsi_signal']}\n"
        f"EMA 8/21/50: {sig['ema8']} / {sig['ema21']} / {sig['ema50']}\n"
        f"Stack: {sig['ema_alignment'].replace('_',' ')} — {sig['ema_signal']}\n"
        f"BB: {_p(sig['bb_lower'])} / {_p(sig['bb_upper'])} — {sig['bb_signal']}\n"
        f"VWAP: {_p(sig['vwap'])} ({sig['vwap_position']})\n"
        f"5m Trend: {sig['trend_5m']} | Structure: {sig['structure_5m']}\n"
        f"Volume: {sig['volume_ratio']}x — {sig['volume_signal']}\n"
        f"VIX: {sig['vix']} — {sig['regime']}\n"
        f"Fear & Greed: {sig['fear_greed']}/100 ({sig['fear_greed_label']})\n\n"
        f"SMC CONFLUENCES\n"
        f"{conf_text}"
        f"{smc_lines}\n\n"
        f"ENTRY\n"
        f"Zone:    {sig['entry']}\n"
        f"Trigger: {sig['entry_trigger']}\n\n"
        f"RISK MANAGEMENT\n"
        f"Stop:  {_p(sig['stop_loss'])} (-{_pct(sig['stop_loss_pct'])})\n"
        f"Why:   {sig['stop_loss_reason']}\n"
        f"Advice: {sig.get('risk_comment','n/a')}\n\n"
        f"TARGETS\n"
        f"TP1: {_p(sig['tp1'])} (+{_pct(sig['tp1_pct'])}) — close 50%\n"
        f"TP2: {_p(sig['tp2'])} (+{_pct(sig['tp2_pct'])}) — close 30%\n"
        f"TP3: {_p(sig['tp3'])} (+{_pct(sig['tp3_pct'])}) — close 20%\n"
        f"R:R: {sig['rr']}\n\n"
        f"ANALYSIS\n"
        f"Catalyst: {sig['news_catalyst']}\n"
        f"{sig['analysis_summary']}\n\n"
        f"INVALIDATION\n"
        f"{sig['invalidation']}"
        f"{event_warn}\n\n"
        f"───────────────────\n"
